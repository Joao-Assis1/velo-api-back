...
- Pagamentos: Asaas (tokenização de cartão, cobranças, transfers e webhooks)
- OCR: Tesseract.js
...
├── payments/          # Asaas: charge, release, dispute, webhooks
├── payment-methods/   # cartoes salvos (asaasCreditCardToken); listagem, default, delete
...
- **Pagamentos:** Asaas cobranças via cartão tokenizado; release via `AsaasService.createTransfer` após aula `completed` + `isValidForCompliance(lesson)=true` (≥50 min, biometria OK, sem disputa); idempotência via SHA-256 derivada de `(subject, action)`.
...
| `ASAAS_API_KEY` | Chave de API da plataforma Asaas |
| `ASAAS_BASE_URL` | URL base do Asaas (sandbox: `https://sandbox.asaas.com/api`) |
| `ASAAS_WEBHOOK_TOKEN` | Token para verificação de webhooks do Asaas |
...
- **Asaas (seed):** Gabriel tem `asaasCustomerId=cus_seed_gabriel` + 1 PaymentMethod seed com `asaasCreditCardToken` (cartão de teste do sandbox Asaas)
