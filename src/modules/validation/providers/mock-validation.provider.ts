...
export class MockValidationProvider implements DocumentValidationProvider {
  ...
  async validateCnh(cnhNumber: string, _cpf: string): Promise<CnhExternalCheck> {
    await new Promise((r) => setTimeout(r, 50));
    const normalized = (cnhNumber ?? '').replace(/\D/g, '');
    return this.knownCnh.get(normalized) ?? { valid: false, status: 'NOT_FOUND' };
  }
  ...
  async matchFaceWithCnh(_cpf: string, _faceImageBase64: string): Promise<FaceMatchResult> {
    await new Promise((r) => setTimeout(r, 50));
